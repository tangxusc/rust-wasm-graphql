...
通过 WIT 级别为每个命令类型导出独立的 `validate-{cmd}` 和 `handle-{cmd}` 函数，
...
impl CommandDiscovery {
    /// 扫描导出函数，配对 validate-X / handle-X，返回命令列表
    pub fn discover(functions: &[ExportedFunction]) -> Result<Vec<CommandDef>, String> {
        ...
        for (cmd_name, validate_fn) in &validates {
            let handle_fn = handles.get(cmd_name).ok_or_else(|| {
                format!("命令 '{}' 缺少对应的 handle-{} 函数", cmd_name, cmd_name)
            })?;
            commands.push(CommandDef {
                name: cmd_name.to_string(),
                validate_fn: validate_fn.wit_name.clone(),
                handle_fn: handle_fn.wit_name.clone(),
            });
        }

        // 检查孤立的 handle（有 handle 但无 validate）
        for cmd_name in handles.keys() {
            if !validates.contains_key(cmd_name) {
                return Err(format!(
                    "命令 '{}' 有 handle-{} 但缺少 validate-{}", 
...
pub struct CommandDef {
    ...
    pub validate_fn: String,    // "validate-create-item"
...
旧方案通过检查固定的 `validate` + `handle` + `apply-events` 三个函数判定聚合模块。
新方案改为：
...
impl CommandGateway {
    pub async fn execute(&self, command: IncomingCommand) -> Result<CommandResult> {
        // 1. 前置 validate（按命令类型路由到对应的 validate-X 函数）
...
impl Guest for Component {
    ...
    fn validate_delete_item(command: Vec<u8>) -> Result<(), String> {
        // delete-item 无需格式校验（reason 可选）
        let _: DeleteItemParams = serde_json::from_slice(&command)
            .map_err(|e| format!("反序列化失败: {e}"))?;
        Ok(())
    }
...
关键变化：Gateway 和 Actor 分别调用 `validate-{cmd}` 和 `handle-{cmd}`，而非通用的 `validate`/`handle`。

## 启动时校验

Host 加载 WASM 组件时执行严格校验，启动失败优于运行时错误：
...
impl WasmRegistry {
    fn validate_aggregate_module(&self, engine: &WasmEngine) -> Result<(), String> {
        ...
        // 1. 必须有 apply-events
        if !functions.iter().any(|f| f.wit_name == "apply-events") {
            return Err("聚合模块缺少 apply-events 函数".into());
        }

        // 2. validate-X 和 handle-X 必须成对出现
        let commands = CommandDiscovery::discover(functions)?;
        if commands.is_empty() {
            return Err("聚合模块至少需要一个命令（validate-X + handle-X 对）".into());
        }
...
| `handle-X` 无对应 `validate-X` | 启动失败，日志报告缺失函数 |
| 缺少 `apply-events` | 不识别为聚合模块（降级为普通查询模块） |
...
| `validate-X` 返回 Err | 返回校验错误，无副作用 | 400 |
